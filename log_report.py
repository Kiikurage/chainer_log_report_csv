...
def _dump_json(log, f):
    json.dump(log, f, indent=4)


def _dump_csv(log, f):
    import pandas
    pandas.DataFrame(data=log).to_csv(f)
...
class LogReport(extension.Extension):
    ...
    def __init__(self, keys=None, trigger=(1, 'epoch'), postprocess=None,
        ...
        if type not in _log_adapter_list:
            msg = 'log type {0} is invalid.'.format(type)
            raise ValueError(msg)

        self._dump = _log_adapter_list[type]
